...
class Transaction(ViewSet):
    ...
    def check_perform(self, request):
        serializer = CheckPerformTransactionSerializer(data=request.data)
        if not serializer.is_valid():
            return Response({"jsonrpc": "2.0", "error": {"code": -32700, 'message': serializer.errors}},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if not user:
            return Response({"jsonrpc": "2.0", "error": {"code": -31050, 'message': "User not found"}},
                            status=status.HTTP_400_BAD_REQUEST)

        # Agar userda active subscription mavjud bo'lsa, qayta obuna qilishni bloklash
        if user.active_subscriptions:
            return Response({"jsonrpc": "2.0", "error": {"code": -31050, 'message': "You are already subscribed."}},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def perform_transaction(self, request):
        ...
        transaction_id = serializer.validated_data.get('params', {}).get('id')
        if not transaction_id:
            return Response({"jsonrpc": "2.0", "error": {"code": -32700, 'message': 'Transaction ID is missing.'}},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if not transaction_data:
            return Response({"jsonrpc": "2.0", "error": {"code": -31003, 'message': 'Transaction does not exist.'}},
                            status=status.HTTP_400_BAD_REQUEST)

        if transaction_data and transaction_data.state != 1:
            return Response({"jsonrpc": "2.0", "error": {"code": -31003, 'message': 'Transaction state is invalid.'}},
                            status=status.HTTP_400_BAD_REQUEST)
