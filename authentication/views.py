...
class PasswordViewSet(ViewSet):
    ...
    def send_token_password(self, request):
        ...
        otp = check_otp_attempts(OTP.objects.filter(otp_key=serializer.validated_data['otp_key']).first(), serializer.validated_data['otp_code'])
    ...
    def verify_with_token(self, request):
        ...
        user = User.objects.filter(resettoken__token=serializer.validated_data['otp_token']).first()
    ...
    def change_password(self, request):
        ...
        validate_data = UserSerializer(user, data={'password': serializer.validated_data['new_password']}, partial=True)
