...
class TestVerseViewSet(APITestCase):
    def setUp(self):
        ...
        self.verse2 = Verse.objects.create(chapter=self.chapter, number=2, description='Description of Test Verse 2',
                                           text='Another', text_arabic='Arabic Text of Verse 2')

    def test_verse_list(self):
        response = self.client.get(reverse('search_verse'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['result'][0]['chapter_name'], 'Test Chapter')
        self.assertEqual(response.data['result'][0]['number'], 2)
        self.assertNotEqual(response.data['result'][0]['id'], 1)
        response2 = self.client.get(reverse('search_verse'), kwargs={'q': 'Another'})
        self.assertEqual(response2.status_code, status.HTTP_200_OK)
        self.assertEqual(response2.data['result'][0]['chapter_name'], 'Test Chapter')
